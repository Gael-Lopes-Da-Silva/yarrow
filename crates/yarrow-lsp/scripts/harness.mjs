#!/usr/bin/env node
/**
 * Minimal LSP protocol harness for yarrow-lsp (Stage 19).
 *
...
function usage() {
  console.error("usage: node crates/yarrow-lsp/scripts/harness.mjs [pull-diagnostics]");
  process.exit(2);
}
...
async function scenarioPullDiagnostics(client) {
  ...
  const hit = flat.find((d) => {
    const code = d && d.code;
    return code === "E373" || code === 373 || String(code) === "E373";
  });
  if (!hit) {
    throw new Error(
