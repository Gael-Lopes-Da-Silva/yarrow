...
function startServer(extraEnv = {}) {
	...
	const ready = new Promise((resolve, reject) => {
		const timer = setTimeout(() => {
			reject(
				new Error(
					`timeout waiting for listen line; stderr:\n${stderr}`,
				),
			);
		}, 120_000);
...
function digDiagnostics(report) {
	...
	if (report.full && Array.isArray(report.full.items))
		return report.full.items;
...
async function scenarioDefinitionRequire(client) {
	const fixture = path.join(REPO_ROOT, "docs/examples/valid/12_modules.yar");
