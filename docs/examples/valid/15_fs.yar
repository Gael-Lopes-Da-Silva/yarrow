...
main function do
	"/tmp/yarrow_fs_stage23.txt" 'w' fs.open_file call unwrap
	out const File
	...
	"/tmp/yarrow_fs_stage23.txt" 'r' fs.open_file call unwrap
	inp const File

	inp borrow
	fs.read_file call unwrap
	got const string

	inp borrow
	fs.close_file call

	got "stage23-ok" str.compare call
	0 == if
