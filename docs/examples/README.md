...
Interpreter (Stage 37): `01`–`15` except `00_grammar_tour.yar` interpret with stdout matching JIT (unsafe/`pointer<T>`/`move`/fs). Gate: `cargo run -p yarrow_core --example check_interpret`. The grammar tour stays `E393` (out of scope).
